  ...
  operationTypeColRender(op) {
    switch (op.type) {
      ...
      case 'create_passive_offer':
        ...
        return <span>
          {action} {this.amount(op.amount, op.selling_asset_type, op.selling_asset_code, op.selling_asset_issuer)} for {op.buying_asset_type == "native" ? <i>XLM</i> : <AssetLink horizonURL={this.props.horizonURL} code={op.buying_asset_code} issuer={op.buying_asset_issuer} />}
        </span>;
      case 'account_merge':
        return <span>&raquo; <AccountBadge horizonURL={this.props.horizonURL} id={op.into} /></span>
      case 'manage_data':
        ...
        <tbody>
          {
            this.state.operations.map(op => {
              return <tr key={op.id}>
                <td><AccountBadge horizonURL={this.props.horizonURL} id={op.source_account} known={this.props.account} /></td>
