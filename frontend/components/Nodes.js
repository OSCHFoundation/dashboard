...
export default class Nodes extends React.Component {
  ...
  render() {
    ...
    list = list.slice((this.state.page-1)*NODES_PER_PAGE, (this.state.page)*NODES_PER_PAGE);
    ...
    return (
      <div>
          ...
          <div className="mui--pull-left">
            ...
            <Button variant="flat" size="small" color="primary"
              ...
              onClick={this.setOrder.bind(this, ORDER_ALPHABETICAL)}>
                <i className="material-icons mui--visible-xs-inline">sort_by_alpha</i>
            ...
            {this.state.page > 1 ?
              <Button variant="flat" size="small" color="primary"
                onClick={this.movePage.bind(this, -1)}>
                  &laquo; Prev<span className="mui--hidden-sm mui--hidden-xs"> page</span>
              </Button>
              : null}
            {this.state.page < commonNodes.length / NODES_PER_PAGE ?
              <Button variant="flat" size="small" color="primary"
                onClick={this.movePage.bind(this, 1)}>
                  Next<span className="mui--hidden-sm mui--hidden-xs"> page</span> &raquo;
              </Button>
              : null}
          </div>
        </div>
        ...
        <div className="mui-col-md-12">
          {
            Object.values(list)
              .map(node => <Node key={node.id} data={node} uptime={this.state.uptimeData[node.id]} />)
          }
        </div>
      </div>
    );
